<template>
  ...
  >
    <!-- single quote -->
    <div v-if="quotes.length === 1" class="card" ref="singleQuote">
      <div
        class="border relative rounded-t-lg"
        :class="{
          'bg-white': type === 'default',
          'bg-gray-600 border-gray-600': type === 'defaultDark',
        }"
      >
        <div
          class="absolute inset-0"
          :class="{
            'text-gray-600': type === 'default',
            'text-gray-100': type === 'defaultDark',
          }"
        >
          <svg
        ...
        <div class="card-body m-8">
          ...
          ></div>
          <div
            v-if="quotes[0].comment && quotes[0].comment !== '<p></p>'"
            class="m-4"
          >
            ...
            <div
              ...
              :class="{
                'text-blue-900': type === 'default',
                'text-blue-300': type === 'defaultDark',
              }"
              v-html="quotes[0].comment"
            ></div>
          </div>
        </div>
      </div>
      ...
      >
        <div class="left flex items-end">
          ...
          <div class="ml-2 text-xs text-gray-400 space-y-1">
            ...
            <p v-if="show.chapter && quotes[0].chapter">
              章节：{{ quotes[0].chapter }}
            </p>
            <p v-if="show.location && quotes[0].location">
              页码：{{ quotes[0].location }}
            </p>
          </div>
        </div>
